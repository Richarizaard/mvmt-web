...
const ContentCard = ({ title, desc, img }: ContentCardProps) => {
  return (
    <div className="w-auto flex items-center flex-col sm:p-2 m-2 cursor-pointer rounded-3xl shadow-black hover:shadow-lg group animate-fadeIn">
      <img className="rounded-3xl sm:p-2 py-2" src={img} alt="" />
      <MvmtHeader3 className="pb-1 md:pb-2">{title}</MvmtHeader3>
      <MvmtParagraphMedium className="sm:p-2 py-2">{desc}</MvmtParagraphMedium>
    </div>
  )
}
